...
function ReviewMode({ scenario, questions, isParadoxMode, onGoToAnalysis }) {
    ...
    const handleEmailShare = () => {
        ...
        let body = `Hi! I just used a Really Useful Thing (RUT) from CharityOps.org to question storm a challenge.\n\n`;
        body += `Challenge: ${scenario}\n`;
        if (isParadoxMode) body += `Mode: Roll the dice for a 13\n`;
        body += `\n--- TOP 3 QUESTIONS ---\n`;
        if (starred.length === 0) {
            body += `(No questions were starred)\n`;
        } else {
            starred.forEach((q, i) => body += `${i + 1}. ${q.text}\n`);
        }

        body += `\n--- OTHER QUESTIONS ---\n`;
        others.forEach((q, i) => body += `${i + 1 + starred.length}. ${q.text}\n`);
    ...
    return (
        <div className="fade-in">
            ...
            <div className="review-footer">
                <div className="action-row" style={{ display: 'flex', gap: '0.75rem', width: '100%', flexWrap: 'wrap' }}>
                    <button onClick={handleWhatsAppShare} className="whatsapp-btn">
                        <MessageCircle size={18} />
                        WhatsApp
                    </button>
                    <button onClick={handleEmailShare} className="download-btn">
                        <Mail size={18} />
                        Email
                    </button>
                    ...
                    <button
                        ...
                        style={{ flex: 1.5, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', minWidth: '200px' }}
                    >
