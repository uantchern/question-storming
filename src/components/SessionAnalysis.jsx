            ...
            <div className="report-card">
                ...
                {isParadoxMode && (
                    <div className="report-section paradox-summary-section">
                        <h3><Zap size={18} className="active-icon" /> 13-Dice Interference Summary</h3>
                        <div className="paradox-insight-card">
                            <p>Reality was successfully fractured {questions.filter(q => q.paradoxConstraint).length} times during this session.</p>
                            <div className="interference-stats">
                                <span>Anomaly Intensity: {Math.round((questions.filter(q => q.paradoxConstraint).length / (totalQuestions || 1)) * 100)}%</span>
                                <span>Singularity Stability: {Math.max(0, 100 - (questions.filter(q => q.paradoxConstraint).length * 9))}%</span>
                            </div>
                            <div className="derivation-footnote" style={{ marginTop: '1rem', fontSize: '0.75rem', opacity: 0.7, borderTop: '1px solid rgba(255,0,255,0.2)', paddingTop: '0.75rem' }}>
                                <strong>Calculated Derivations:</strong>
                                <br />
                                • <strong>Anomaly Intensity</strong> is the ratio of reality-fractured questions to total volume ({questions.filter(q => q.paradoxConstraint).length} of {totalQuestions}).
                                <br />
                                • <strong>Singularity Stability</strong> is an inverse pressure score, decaying by 9% for every paradox injection successfully processed.
                            </div>
                        </div>
                    </div>
                )}
